MODULE types
    ...
    !     04/08/19    Keith Wilcox         Added Session Loop and ToUpper subroutine
    !
    IMPLICIT NONE
...
LOGICAL FUNCTION lt_city (a, b)
    ...
    ! Data dictionary: declare calling parameter types & definitions
    TYPE (personal_info), INTENT(IN) :: a, b
    CHARACTER(len=12) :: mystrA
    CHARACTER(len=12) :: mystrB
    ! Make comparison.
    mystrA = a%city
    mystrB = b%city
    CALL To_upper(mystrA)
    CALL To_upper(mystrB)
    lt_city = LLT ( mystrA, mystrB )
